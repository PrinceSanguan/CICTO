...
- [x] **The `verified` middleware on every protected route is inert: User does not implement MustVerifyEmail, so anyone can self-register and reach every authenticated screen with an unverified address** — *fixed 2026-08-23, the day outgoing mail became real.* `User` declares `implements MustVerifyEmail` now, so registration sends the verification link and the middleware on all six protected route groups enforces what the route files had been claiming since Phase 1. It could not honestly be closed earlier: under `MAIL_MAILER=log`, declaring the interface would have locked every newly registered account out of a system with no way to send it a link, which is why the _Fix:_ below pairs it with dropping self-registration. The operator configured Google SMTP on 2026-08-23 and that constraint went away, so registration was kept — throttled by `ThrottlePublicRegistration` — rather than removed. All five existing users were already verified, so nobody was locked out.  
...
- [x] **The deploy-time super-admin command uses Password::uncompromised(), which AppServiceProvider explicitly rejects for this deployment** — *stale when written, and a duplicate of the `consistency` entry further down.* Verified against the source and ticked 2026-08-20.  
...
- [x] **Password reset claims the email was sent when MAIL_MAILER=log, and writes the working reset token into the log file** — *fixed 2026-08-20 alongside the B3 answer.* `RequireOutgoingMail` refuses `password.email` when the mailer is `log`/`array`, so no token is minted at all; the page drops the form and names the administrator instead, and the refusal reads identically for a known and an unknown address so it cannot be used to enumerate accounts. The `log` mailer now writes to its own 7-day channel (`MAIL_LOG_CHANNEL=mail`) rather than into `stack`, which reduces — and does not remove — the exposure of whatever it does render. `tests/Feature/Auth/MailUnavailableTest.php` pins all of it. **Superseded 2026-08-23:** the operator configured Google SMTP, so `RequireOutgoingMail` passes the POST through, the form is back, and the link is delivered to a person rather than rendered into a log. The guard stays for deployments without a mailer. It had also been the *only* limiter on that route, so `ThrottlePasswordResetRequests` (10 per IP per hour) replaces it — `config/auth.php`'s `'throttle' => 60` is keyed on the email address and does nothing about one client walking a staff list.  
...
- [ ] **Single unrotated log channel at debug level receives every outbound email body, including password-reset links** — *half-superseded 2026-08-23.* `MAIL_MAILER=smtp` means no message body reaches any log channel at all, and `MAIL_LOG_CHANNEL=mail` had already routed the `log` mailer off `stack` on 2026-08-20, so the email-body half no longer describes this deployment. Left open because the rest of it stands on its own: `.env.example` still ships `LOG_STACK=single` and `LOG_LEVEL=debug`, so the main channel is still one unrotated file at debug level.  
...
- [x] **CreateSuperAdminCommand uses uncompromised(), which AppServiceProvider explicitly rejects for this host profile** — *stale when written.* The file has read `Password::min(12)->letters()->numbers()` with a "No uncompromised() check" comment since before this review, and `AccountCommandsTest::test_neither_command_depends_on_an_external_password_service` greps both commands for it. Verified against the source and ticked 2026-08-20 rather than re-fixed.  
